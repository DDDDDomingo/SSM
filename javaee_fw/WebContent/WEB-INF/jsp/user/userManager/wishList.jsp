			...
			<div class="col-md-12">
				<ol class="breadcrumb">
					...
					<a class="btn btn-primary btn-large"
						href="<%=basePath%>saleList/addWishList?id=${userInfo1.userId}"
						style="margin-left: 195px; background-color: #B22222; border-color: #CD0000;">添加心愿</a>
				</ol>
			</div>
					...
					<table>
						<tbody>
							...
							<c:if test="${!empty listF}">
								<c:forEach items="${listF}" var="lf" varStatus="vs">
									<tr>
										<td><img src="<%=basePath%>${lf.wlistImg}"
											style="width:80px;padding-right: 10px;padding-bottom: 15px;"></td>
										<td><c:out value="${lf.wlistName}" /></td>
										<td><c:out value="${lf.plusTime}" /></td>
										<td><a href="<%=basePath%>saleList/editWishList?id=${lf.wlistId}&userId=${lf.userId}">
												<button type="button" class="btn btn-primary btn-sm">
													<span class="glyphicon glyphicon-edit"></span>
												</button>
											</a></td>
										<td><a
												href="<%=basePath%>saleList/removeWishList?id=${lf.wlistId}&userId=${lf.userId}">
					...
					<table>
						<tbody>
							...
							<c:if test="${!empty listT}">
								<c:forEach items="${listT}" var="llt" varStatus="vss">
									<tr>
										<td><img src="<%=basePath%>${llt.wlistImg}"
											style="width:80px;padding-right: 10px;padding-bottom: 15px;" /></td>
										<td><c:out value="${llt.wlistName}" /></td>
										<td><c:out value="${llt.plusTime}" /></td>
										<td><a
												href="<%=basePath%>saleList/removeWishList?id=${llt.wlistId}">
												<button type="button" class="btn btn-danger btn-sm">
													<span class="glyphicon glyphicon-remove"></span>
												</button>
											</a></td>
